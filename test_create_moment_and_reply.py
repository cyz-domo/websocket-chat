...
# 访问创建动态页面
create_moment_url = 'http://127.0.0.1:8000/chat/moments/create/'
response = session.get(create_moment_url)
soup = BeautifulSoup(response.text, 'html.parser')
csrf_token = soup.find('input', {'name': 'csrfmiddlewaretoken'})['value']

# 创建动态
data = {
    'content': '测试动态，用于测试回复评论功能',
    'visibility': 'public',
    'csrfmiddlewaretoken': csrf_token
}

response = session.post(create_moment_url, data=data)
print(f"Create moment status code: {response.status_code}")
...
# 查找第一个动态的ID
soup = BeautifulSoup(response.text, 'html.parser')
moment_div = soup.find('div', class_='moment-card')
if moment_div:
    ...
    print(f"Add comment status code: {response.status_code}")
    print(f"Add comment response: {response.json()}")
    
    # 回复评论
    comment_data = response.json()
    if comment_data.get('status') == 'ok':
        parent_comment_id = comment_data.get('comment', {}).get('id')
        print(f"Found comment with ID: {parent_comment_id}")
